 ...
 * stored inside the mode of a file. This assignment
 * only uses files and directories, but real-world filesystems
 * have more types.
 */
enum FILE_TYPE {
    TYPE_FILE = 1,
...
typedef struct {
     ...
     *    the character and pipe aren't used in this mp
     ...
     */
    uint32_t nlink; /* reference count, free when hit 0 */
 ...
 *
 *  Note that you should only overwrite the bottom RWX_BITS_NUMBER bits.
 *  These lower bits are in the form rwxrwxrwx for owner, group, and others.
 ...
 *
 *  Example:
 *  Suppose the file '/text' contains the following:
 *    Hello.
 ...
 *  buffer with the file's attributes.
 */
