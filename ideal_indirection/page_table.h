...
/**
 * In this assignment, we will simulate 32-bit virtual memory and 32-bit
 * physical memory spaces. There will be two levels of page tables in this
 * system, and each page table will be 4kb (2^12 bytes) in size.
 *
 * We will call the first level page table a "page_directory" and a second level
 * page table a "page_table". Therefore, entries in a page_directory, known as
 * "page_directory_entry", will point to a page_table. Similarly, entries in a
 * page_table, known as "page_table_entry", will point to a data frame.
 *
 * You will be responsible for reading from, and writing to some of the bits
 * in the page directory and page table entries. Take a moment to familiarize
 * yourself with these bit entries and what they represent. Think about when
 * you will access these bit entries during the virtual memory address
 * translation process.
 */
